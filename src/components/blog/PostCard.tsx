...
export default function PostCard({ title, coverImage, date, excerpt, author, slug }: any) {
  return (
    <Link href={`/blog/${slug}`}>
      <a className='flex flex-col overflow-hidden rounded-md border border-zinc-500 shadow-lg transition-all duration-200 hover:border-hover hover:shadow-2xl'>
        <div className='flex-shrink-0'>
          <CoverImage title={title} responsiveImage={coverImage.responsiveImage} slug={slug} />
        </div>
        <div className='flex flex-1 flex-col justify-between border-t border-zinc-500 bg-[url("/services-pattern.webp")] p-6 mix-blend-screen'>
